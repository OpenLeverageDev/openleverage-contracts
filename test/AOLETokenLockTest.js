...
contract("OLETokenLock", async accounts => {
    ...
    it("Take out all at one time, the time has expired: ", async () => {
        ...
        await oleTokenLock.release({from: accounts[1]});
    ...
    it("The withdrawal address is non beneficiary: ", async () => {
        ...
        let oleTokenLock = await OLETokenLock.new(oleToken.address, [accounts[1]], [toWei(100000)], ['1599372311'], [(parseInt(await lastBlockTime()))]);
        await oleToken.transfer(oleTokenLock.address, toWei(100000));
        await assertThrows(oleTokenLock.release({from: accounts[2]}), 'beneficiary does not exist');
    });
    ...
    it("Cash withdrawal before start time:", async () => {
        ...
        await oleToken.transfer(oleTokenLock.address, toWei(100000));
        await assertThrows(oleTokenLock.release({from: accounts[1]}), 'not time to unlock');
    });
    ...
    it("If the withdrawal is completed, is it wrong to withdraw again: ", async () => {
        ...
        await assertThrows(oleTokenLock.release({from: accounts[1]}), 'no amount available');
    });
    ...
    it("Two accounts, two addresses, partial withdrawal: ", async () => {
        ...
        await oleToken.transfer(oleTokenLock.address, toWei(100000));
        await oleTokenLock.release({from: accounts[3]})
    ...
    it("transfer to A error with beneficiary does not exist test", async () => {
        ...
        await assertThrows(oleTokenLock.transferTo(accounts[2], toWei(20000), {from: accounts[3]}), 'beneficiary does not exist');
    });
    ...
    it("transfer to A  error with locked end test", async () => {
        ...
        await assertThrows(oleTokenLock.transferTo(accounts[2], toWei(20000), {from: accounts[1]}), 'locked end');
    });

    it("transfer to a exit account error with to is exist test", async () => {
        let oleToken = await OLEToken.new(accounts[0], accounts[0], 'TEST', 'TEST');
        let oleTokenLock = await OLETokenLock.new(oleToken.address, [accounts[1],accounts[2]], [toWei(100000),toWei(100000)],
        ...
        await assertThrows(oleTokenLock.transferTo(accounts[2], toWei(20000), {from: accounts[1]}), 'to is exist');
    });
    ...
    it("transfer to A succeed test", async () => {
        ...
        await oleTokenLock.transferTo(accounts[2], toWei(20000), {from: accounts[1]});
        await timeMachine.advanceTime(4000);
        await oleTokenLock.release({from: accounts[1]});
        await oleTokenLock.release({from: accounts[2]});
    ...
    it("transfer to A,A transfer to B test", async () => {
        ...
        await oleTokenLock.transferTo(accounts[3], toWei(5000), {from: accounts[2]});
        await timeMachine.advanceTime(2000);
        assert.equal((await oleTokenLock.releaseVars(accounts[1])).end,(await oleTokenLock.releaseVars(accounts[2])).end);
