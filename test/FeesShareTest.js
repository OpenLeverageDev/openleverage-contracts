...
contract("XOLE", async accounts => {
    ...
    beforeEach(async () => {
        ...
        daiOLEDexData = Uni2DexData + "000000000000000000000000" + addressToBytes(dai.address) + addressToBytes(ole.address);
        usdtOLEDexData = Uni2DexData + "000000000000000000000000"+ addressToBytes(usdt.address) + addressToBytes(ole.address);
        daiUsdtDexData = Uni2DexData + "000000000000000000000000"+ addressToBytes(dai.address) + addressToBytes(usdt.address);
    ...
    it("Convert DAI to USDT to OLE ", async () => {
        ...
        await xole.create_lock(toWei(10000), lastbk.timestamp + WEEK);
        assert.equal('10000000000000000000000', (await usdt.balanceOf(xole.address)).toString());
        await xole.convertToSharingToken(toWei(1000), 0,  "0x01" + "000000" + "03" + "000000000000000000000000000000000000" + addressToBytes(dai.address) + addressToBytes(usdt.address) + addressToBytes(ole.address));
        m.log("xOLE USDT balance:", await usdt.balanceOf(xole.address));
        assert.equal('10000000000000000000000', (await usdt.balanceOf(xole.address)).toString());
